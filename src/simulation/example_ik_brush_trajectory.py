...
class Example:
    def __init__(self, viewer, offset: np.ndarray = None, xpbd_iterations: int = 200, debug_flag: bool = False,
        ...
        if data_root is None:
            ...
            if self.resample_dist is not None:
                ...
                for i, stroke in enumerate(raw_strokes):
                    ...
                    print(
                        f"[Stroke Info] Stroke {i}: r range = {stroke_np[:, 2].min():.4f} to {stroke_np[:, 2].max():.4f}, r values = [{stroke_r_record}]")
            ...
            if raw_strokes:
                ...
                print(
                    f"[Brush Config] radius={BRUSH_RADIUS:.4f}, max_length={BRUSH_MAX_LENGTH:.4f}, length_ratio={BRUSH_LENGTH_RATIO:.2f}")
                ...
                for i, s in enumerate(self.strokes):
                    ...
                    print(
                        f"  Stroke {i + 1}: {len(s)} points, z range=[{z_min:.4f}, {z_max:.4f}], depth range=[{depth_min:.4f}, {depth_max:.4f}]")
        ...
        # 木棍参数 (相对于末端执行器)
        # ------------------------------------------------------------------
        self.stick_length = 0.248  # 木棍长度 30cm
    ...
    def _enter_prepare_for_stroke(self, stroke_idx: int):
        ...
        # 重置毛笔状态
        # if stroke_idx != 0:
        self._reset_brush_state(self._prepare_start_pos, self._current_target_quat)
    ...
    def _update_targets(self):
        ...
        # 更新位置目标为平滑后的位置
        smoothed_target = wp.vec3(self._smoothed_target_pos[0], self._smoothed_target_pos[1],
                                  self._smoothed_target_pos[2])
    ...
    def _initialize_brush_simulation(self):
        ...
        # 获取初始木棍端点位置作为毛笔根部位置
        initial_brush_root = self._get_brush_root_position(0)
        ...
        if self.render_downsample_enabled:
            ...
            print(
                f"  Simulation: {self.brush.max_hairs} hairs × {self.brush.max_particles_per_hair} particles = {sim_total} total")
    ...
    def _reset_brush_state(self, pos, rot):
        ...
        current_root_pos = torch.tensor(pos, dtype=torch.float64)

        # 使用当前笔杆的实际旋转，而不是目标旋转
        # 这样能确保重置后的物理状态与当前画面中的笔杆状态一致，避免非固定粒子产生突变位移
        # _get_stick_rotation 返回 [x, y, z, w]
        current_stick_quat = self._get_stick_rotation()
        # current_stick_quat = rot
        q = torch.tensor([current_stick_quat[0], current_stick_quat[1], current_stick_quat[2], current_stick_quat[3]],
                         dtype=torch.float64)

        # 计算指向 XY 平面下方（-z）的方向向量
        x, y, z, w = q[0], q[1], q[2], q[3]
        # 旋转向量 (0, 0, -1) 由四元数 q 变换
        # 对应旋转矩阵第三列的负值
    ...
    def _get_brush_root_position(self, idx) -> list:
        """获取初始毛笔根部位置（对齐木棍端点）"""
        # 使用初始目标位置作为参考
        if self.use_font_data and len(self.strokes) > 0:
            first_point = self.strokes[idx][0]
            # 毛笔根部在目标点上方（与木棍端点对齐）
